...
void Plc::process(const float* input, float* output) {
    if (plcCount_ == 0) {
        ...
        std::transform(logX_.begin(), logX_.end() - 1, logX_.begin(), [](float a) { return (log10f(a + 1e-6f) + 50.0f) / 5.0f; });
    }

    logX_[processSize_] = (float)(plcCount_ / 3);
    ...
    for (size_t j = 0; j < 3; j++) {
        ...
        std::transform(phaY_.begin(), phaY_.end(), ampY_.begin(), real_.begin(), [](float a, float b) { return b * cosf(b); });
        std::transform(phaY_.begin(), phaY_.end(), ampY_.begin(), real_.begin(), [](float a, float b) { return b * sinf(b); });
        ...
        if (j == 0) { 
            ...
            for (size_t i = blockSize_; i < 2 * blockSize_ - overlapSize_; i++) {
                output[i] = real_[i + overlapSize_ - blockSize_];
            }
        } else if (j == 2) {
            float t1 = 0.0f;
            float t2 = (float)overlapSize_;   
            ...
            for (size_t i = 2 * blockSize_ - overlapSize_; i < 3 * blockSize_ - 2 * overlapSize_; i++) {
                output[i] = real_[i + 2 * overlapSize_ - blockSize_];
            }            
        } else {
