...
absl::optional<PrefixParser::PrefixState> PrefixParser::ParsePrefixUpToSvcExtension(
  ...
  uint32_t svc_extension_flag = buffer->ReadBits(1);
  if (!svc_extension_flag)
    return OptionalPrefix();

  // idr_flag: u(1)
  // RETURN_EMPTY_ON_FAIL(buffer->ReadBits(&svc_extension.idr_flag, 1));
  svc_extension.idr_flag = buffer->ReadBits(1);
  // priority_id: u(6)
  // RETURN_EMPTY_ON_FAIL(buffer->ReadBits(&svc_extension.priority_id, 6));
  svc_extension.priority_id = buffer->ReadBits(6);
  // no_inter_layer_pred_flag: u(1)
  // RETURN_EMPTY_ON_FAIL(buffer->ReadBits(&svc_extension.no_inter_layer_pred_flag, 1));
  svc_extension.no_inter_layer_pred_flag = buffer->ReadBits(1);
  // dependency_id: u(3)
  // RETURN_EMPTY_ON_FAIL(buffer->ReadBits(&svc_extension.dependency_id, 3));
  svc_extension.dependency_id = buffer->ReadBits(3);
  // quality_id: u(4)
  // RETURN_EMPTY_ON_FAIL(buffer->ReadBits(&svc_extension.quality_id, 4));
  svc_extension.quality_id = buffer->ReadBits(4);
  // temporal_id: u(3)
  // RETURN_EMPTY_ON_FAIL(buffer->ReadBits(&svc_extension.temporal_id, 3));
  svc_extension.temporal_id = buffer->ReadBits(3);
  // use_ref_base_pic_flag: u(1)
  // RETURN_EMPTY_ON_FAIL(buffer->ReadBits(&svc_extension.use_ref_base_pic_flag, 1));
  svc_extension.use_ref_base_pic_flag = buffer->ReadBits(1);
  // discardable_flag: u(1)
  // RETURN_EMPTY_ON_FAIL(buffer->ReadBits(&svc_extension.discardable_flag, 1));
  svc_extension.discardable_flag = buffer->ReadBits(1);
  // output_flag: u(1)
  // RETURN_EMPTY_ON_FAIL(buffer->ReadBits(&svc_extension.output_flag, 1));
  svc_extension.output_flag = buffer->ReadBits(1);
